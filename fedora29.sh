...
if [ "$first" != 1 ];then
    if [ ! -f "fedora.tar.xz" ]; then
        ...
        if [ "$(dpkg --print-architecture)" = "aarch64" ];then
            wget https://us.images.linuxcontainers.org/images/fedora/29/arm64/default/20190417_20:33/rootfs.tar.xz -O fedora.tar.xz
        elif [ "$(dpkg --print-architecture)" = "arm" ];then
            wget https://us.images.linuxcontainers.org/images/fedora/29/armhf/default/20190417_20:33/rootfs.tar.xz -O fedora.tar.xz
        elif [ "$(dpkg --print-architecture)" = "x86_64" ];then
            wget https://us.images.linuxcontainers.org/images/fedora/29/amd64/default/20190417_20:42/rootfs.tar.xz -O fedora.tar.xz
...
cp fedora-fs/root/.bashrc fedora-fs/root/.bashrc.backup
echo "bash /data/data/com.termux/files/home/fedora-in-termux/script.sh && exit" > fedora-fs/root/.bashrc
./start.sh
cp fedora-fs/root/.bashrc.backup fedora-fs/root/.bashrc
