...
if [ "$first" != 1 ];then
    if [ ! -f "fedora.tar.xz" ]; then
        ...
        if [ "$(dpkg --print-architecture)" = "aarch64" ];then
            wget https://us.images.linuxcontainers.org/images/fedora/33/arm64/default/20210409_20:33/rootfs.tar.xz -O fedora.tar.xz
        elif [ "$(dpkg --print-architecture)" = "x86_64" ];then
            wget https://us.images.linuxcontainers.org/images/fedora/33/amd64/default/20210409_20:33/rootfs.tar.xz -O fedora.tar.xz
